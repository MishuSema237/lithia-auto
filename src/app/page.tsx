        ...
        {currentCar && (
          <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8 h-full flex flex-col pt-32 pb-20">
            ...
            <div className="mt-auto relative z-20">
              {/* The Search Bar Box */}
              <div className="bg-white rounded-2xl p-4 mt-8 shadow-none border border-light-300 flex flex-col md:flex-row gap-4 items-center relative z-10 w-full max-w-5xl">
        ...
        <div className="flex space-x-8 mb-10 border-b border-light-200">
          {['All', 'SUV', 'Hatchback', 'Sedan'].map(type => (
            <button
              ...
              className={`pb-4 text-[15px] font-bold transition-all ${selectedBodyType === type ? 'text-navy-900 border-b-2 border-gold-500' : 'text-navy-500 hover:text-navy-900'}`}
            >
              {type}
            </button>
          ))}
      ...
      <section className="py-24 bg-navy-50 overflow-hidden" >
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {isInventoryLoading ? (
                ...
                <div key={car._id || car.id || idx} className={`absolute inset-0 transition-all duration-1000 ease-in-out ${idx === currentLatestPick ? 'opacity-100 scale-100 z-10' : 'opacity-0 scale-105 z-0'}`}>
                  ...
                  <div className="absolute inset-0 bg-gradient-to-r from-navy-900/90 via-navy-900/40 to-transparent flex items-center p-8 md:p-20">
                    <div className="max-w-xl">
                      ...
                      <Link href={`/inventory/${car._id || car.id}`} className="inline-flex items-center bg-gold-500 hover:bg-white text-navy-900 font-extrabold px-12 py-5 rounded-2xl transition-all text-sm uppercase tracking-[0.1em] group/btn">
            ...
            <div className="text-center py-20 text-navy-500 font-bold">No latest picks available.</div>
          )}
        </div>
      </section>
      ...
      <section className="pt-12 pb-24 container mx-auto px-4 sm:px-6 lg:px-8" >
...

        {
          isBlogsLoading ? (
            ...
            <div className="text-center py-20 text-navy-500 font-bold">No blog posts available.</div>
          )
        }
      </section>
