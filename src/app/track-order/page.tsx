...
function TrackOrderContent() {
    ...
    const [orderStatus, setOrderStatus] = useState<null | 'pending' | 'processing' | 'shipped' | 'delivered' | 'not_found'>(null);
    ...
    const statusSteps = [
        { id: 'pending', label: 'Order Placed', icon: <ShoppingBag className="w-5 h-5" />, date: 'Today, 10:30 AM' },
        { id: 'processing', label: 'Processing', icon: <Clock className="w-5 h-5" />, date: 'Expected: 1 day' },
        { id: 'shipped', label: 'Out for Delivery', icon: <Truck className="w-5 h-5" />, date: 'Expected: 3 days' },
        { id: 'delivered', label: 'Delivered', icon: <CheckCircle2 className="w-5 h-5" />, date: 'TBD' },
    ];

    const getCurrentStepIndex = () => {
        if (!orderStatus || orderStatus === 'not_found') return -1;
                            ...
                            <div className="flex flex-col md:flex-row justify-between gap-8 relative z-10">
                                ...
                                <div className="text-left md:text-right space-y-4">
                                    <div>
                                        ...
                                        <div className="text-2xl font-bold text-white">October 24, 2024</div>
                                    </div>
                                    <div className="flex items-center md:justify-end text-navy-200 text-sm font-medium">
                                        <MapPin className="w-4 h-4 mr-2" /> Destination: Beverly Hills, CA
                                    </div>
                                </div>
                            </div>
                        </div>
